...
// ============== Occasions Widget (مناسبت‌ها: تولد و سالگرد) ==============
interface Occasion {
  customerId: string
  customerName: string
...
interface OccasionsData {
  occasions: Occasion[]
  stats: { total: number; today: number; thisWeek: number; birthdays: number; anniversaries: number }
}

export function OccasionsWidget() {
  const api = useApi()
  const apiRef = React.useRef(api)
  apiRef.current = api
  const { openCustomer } = useWorkspace()
  const qc = useQueryClient()
  const [selectedOccasion, setSelectedOccasion] = React.useState<Occasion | null>(null)
  const [smsOpen, setSmsOpen] = React.useState(false)

  const { data, isLoading } = useQuery<OccasionsData>({
    queryKey: ["occasions", 30],
    queryFn: () => apiRef.current.get<OccasionsData>("/api/occasions?days=30"),
    staleTime: 5 * 60 * 1000,
  })
  ...
  const stats = data?.stats ?? { total: 0, today: 0, thisWeek: 0, birthdays: 0, anniversaries: 0 }
      ...
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          ...
          <Badge variant="outline" className="text-pink-600">
            {toPersianDigits(stats.total)} مناسبت
          </Badge>
        </div>
        {stats.today > 0 && (
          <CardDescription className="text-pink-600 dark:text-pink-400">
            🎉 {toPersianDigits(stats.today)} مناسبت امروز
          </CardDescription>
        )}
      </CardHeader>
      <CardContent>
        {occasions.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-8 text-center">
            ...
            <p className="mt-2 text-sm text-muted-foreground">
              در ۳۰ روز آینده مناسبتی ثبت نشده.
            </p>
            <p className="mt-1 text-xs text-muted-foreground">
              برای مشتریان تاریخ تولد یا سالگرد ازدواج ثبت کنید تا اینجا نمایش داده شود.
            </p>
          </div>
        ) : (
          <ScrollArea className="max-h-80">
            <div className="space-y-1.5">
              {occasions.slice(0, 15).map((o) => (
                <OccasionRow
                  key={`${o.customerId}-${o.type}`}
                  occasion={o}
                  onOpen={() => openCustomer(o.customerId)}
                  onSendSms={() => {
              ...
              {occasions.length > 15 && (
                <div className="pt-2 text-center text-xs text-muted-foreground">
                  و {toPersianDigits(occasions.length - 15)} مناسبت دیگر...
                </div>
              )}
            </div>
          </ScrollArea>
      ...
      {smsOpen && selectedOccasion && (
        <OccasionSmsDialog
          ...
          onSent={() => {
            ...
            toast.success("پیامک مناسبت ارسال شد")
          }}
        />
      )}
    </Card>
...
function OccasionRow({ occasion, onOpen, onSendSms }: {
  occasion: Occasion
  onOpen: () => void
  onSendSms: () => void
}) {
  const isBirthday = occasion.type === "birthday"
  const isToday = occasion.daysLeft === 0

  return (
    <div
      className={cn(
        "flex items-center justify-between rounded-md border p-2 transition-colors hover:bg-accent/50",
        isToday && "bg-pink-50 dark:bg-pink-950/20 border-pink-200 dark:border-pink-800",
        isBirthday ? "border-l-4 border-l-pink-300 dark:border-l-pink-700" : "border-l-4 border-l-rose-300 dark:border-l-rose-700"
      )}
    >
      <button
        onClick={onOpen}
      ...
      >
        ...
        <div className="min-w-0">
          <div className="truncate text-sm font-medium">{occasion.customerName}</div>
          <div className="text-xs text-muted-foreground">
            {isBirthday
              ? `تولد${occasion.age ? ` ${toPersianDigits(occasion.age)} سالگی` : ""}`
              : `سالگرد ازدواج${occasion.yearsMarried ? ` ${toPersianDigits(occasion.yearsMarried)} سالگی` : ""}`
            }
          </div>
        </div>
      </button>
      <div className="flex shrink-0 items-center gap-1.5">
        <Badge variant="outline" className={cn(
          ...
          {isToday ? "امروز" : `${toPersianDigits(occasion.daysLeft)} روز`}
        </Badge>
        {occasion.customerPhone && (
          <Button
            size="sm"
            variant="ghost"
            className="h-7 w-7 p-0 text-pink-600 hover:text-pink-700"
            onClick={onSendSms}
            title="ارسال پیامک تبریک"
          >
...
function OccasionSmsDialog({ occasion, onClose, onSent }: {
  occasion: Occasion
  onClose: () => void
  onSent: () => void
}) {
  ...
  const [message, setMessage] = React.useState(
    occasion.type === "birthday"
      ? `سلام ${occasion.customerName} عزیز\nتولد${occasion.age ? ` ${toPersianDigits(occasion.age)} سالگی` : ""}تون مبارک! 🎂\nبا آرزوی سلامتی و شادی\nعکاسی نسیم`
      : `سلام ${occasion.customerName} عزیز\nسالگرد ازدواج${occasion.yearsMarried ? ` ${toPersianDigits(occasion.yearsMarried)} سالگی` : ""}تون مبارک! 💍\nبا آرزوی خوشبختی\nعکاسی نسیم`
  )
  const [sending, setSending] = React.useState(false)

  const handleSend = async () => {
    if (!occasion.customerPhone) {
      toast.error("شماره موبایل مشتری ثبت نشده")
      return
    }
    if (!message.trim()) {
    ...
    try {
      await api.post("/api/sms/send", {
        receptor: occasion.customerPhone,
        message: message.trim(),
        customerId: occasion.customerId,
      })
      onSent()
    } catch (e: any) {
      const msg = e?.message || "خطا در ارسال پیامک"
      toast.error(msg)
    } finally {
      setSending(false)
    }
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            ارسال پیامک {occasion.type === "birthday" ? "تولد" : "سالگرد"}
          </DialogTitle>
          <DialogDescription>
            به: {occasion.customerName} ({occasion.customerPhone})
          </DialogDescription>
        </DialogHeader>
        <div className="space-y-3">
          <div>
            ...
            <div className="mt-1 text-xs text-muted-foreground">
              تعداد کاراکتر: {toPersianDigits(message.length)}
            </div>
          </div>
        </div>
        <DialogFooter>
